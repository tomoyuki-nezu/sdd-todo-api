...
│   ├── settings.json           #   セキュリティ等（共有）
│   ├── settings.local.json     #   ローカル設定（任意）
│   ├── hooks/                  #   PreToolUse 等
│   └── skills/                 #   スキル（各フォルダに SKILL.md）
│       ├── python-lambda/      #     Lambda コード生成ルール
│       └── sam-architect/      #     SAM テンプレート生成ルール
├── .github/
│   └── workflows/
...
| スキル | トリガー | 内容 |
|---|---|---|
| `python-lambda/SKILL.md` | `src/` 配下のコードを扱う時 | ハンドラ構造、型アノテーション、ログ規約 |
| `sam-architect/SKILL.md` | `template.yaml` を扱う時 | IAM 最小権限、環境変数注入、sam validate 必須 |
...
- ワイルドカード (`*`) の使用禁止（`.claude/skills/sam-architect/SKILL.md` で規定）
...
| `.claude/skills/*/`（各 `SKILL.md`） | 半汎用〜固有 | 汎用スキルは流用、プロジェクト用フォルダはスタックに合わせて修正 |
